...
<body>
  ...
  <form method="POST" action="<?php $_SERVER['PHP_SELF']; ?>">
    <div class="container">
      ...
      <input type="text" placeholder="Enter Task Name" name="task_name" value="<?php echo $task['task_name']; ?>"
        required>
      <label for="show">Task Description</label>
      <input type="text" placeholder="Enter Task Description" name="task_description"
        value="<?php echo $task['task_description']; ?>" required>
      <label for="show">Task Due Date</label>
      <input type="date" placeholder="Select Due Date" name="task_duedate" value="<?php echo $task['task_duedate']; ?>"
        required>
      <label for="show">Task Status</label>
      <select name="task_status">
        <option value="incomplete" <?php if ($task['task_status'] == 'incomplete')
          echo 'selected'; ?>>incomplete</option>
        <option value="in progress" <?php if ($task['task_status'] == 'in progress')
          echo 'selected'; ?>>in progress
        </option>
        <option value="complete" <?php if ($task['task_status'] == 'complete')
